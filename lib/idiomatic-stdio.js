...
var 
     SELECTOR=process._console_selector || 'stdout',
     IDIOMS = {
	 'diagnostics': function(c) {
	     rebind(process.stderr, c);
	 },
	 'data': function(c) {
	     rebind(process.stdout, c);
	 }
     };
...
if (console) {
    ...
    exports.console.options = function(config) {
	var tweaked={};
	...
	if (config.idiom && IDIOMS[config.idiom]) {
	    IDIOMS[config.idiom](tweaked);
	}
